...
// follower.
func (repgen *reportGenerationState) messageObserveReq(msg MessageObserveReq, sender commontypes.types.OracleID) {
	...
	if msg.Epoch != repgen.e {
		repgen.logger.Debug(dropPrefix+"wrong epoch",
			commontypes.logger.LogFields{"round": repgen.followerState.r, "msgEpoch": msg.Epoch},
		)
		return
	}
	if sender != repgen.l {
		...
		repgen.logger.Warn(dropPrefix+"non-leader",
			commontypes.logger.LogFields{"round": repgen.followerState.r, "sender": sender})
		return
	}
	if msg.Round <= repgen.followerState.r {
		...
		repgen.logger.Debug(dropPrefix+"earlier round",
			commontypes.logger.LogFields{"round": repgen.followerState.r, "msgRound": msg.Round})
		return
	}
	if int64(repgen.config.RMax)+1 < int64(msg.Round) {
		...
		repgen.logger.Warn(dropPrefix+"out of bounds round",
			commontypes.logger.LogFields{"round": repgen.followerState.r, "rMax": repgen.config.RMax, "msgRound": msg.Round})
		return
	}
	...
	if err != nil {
		repgen.logger.Error("messageObserveReq: could not make SignedObservation observation", commontypes.logger.LogFields{
			"round": repgen.followerState.r,
			"error": err,
		})
		return
	...
	if err := so.Verify(repgen.followerReportContext(), repgen.privateKeys.PublicKeyOffChain()); err != nil {
		repgen.logger.Error("MakeSignedObservation produced invalid signature:", commontypes.logger.LogFields{
			"round": repgen.followerState.r,
			"error": err,
		})
		return
	...
	repgen.logger.Debug("sent observation to leader", commontypes.logger.LogFields{
		"round":       repgen.followerState.r,
		"observation": value,
	})
...
func (repgen *reportGenerationState) messageReportReq(msg MessageReportReq, sender commontypes.types.OracleID) {
	// Each of these guards get their own if statement, to ease test-coverage
	// verification
	if repgen.e != msg.Epoch {
		repgen.logger.Debug("messageReportReq from wrong epoch", commontypes.logger.LogFields{
			"round":    repgen.followerState.r,
			"msgEpoch": msg.Epoch})
		return
	}
	if sender != repgen.l {
		// warn because someone *from this epoch* is trying to usurp the lead
		repgen.logger.Warn("messageReportReq from non-leader", commontypes.logger.LogFields{
	...
	if repgen.followerState.r != msg.Round {
		...
		repgen.logger.Debug("messageReportReq from wrong round", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgRound": msg.Round})
		return
	}
	if repgen.followerState.sentReport {
		repgen.logger.Warn("messageReportReq after report sent", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgRound": msg.Round})
		return
	}
	if repgen.followerState.completedRound {
		repgen.logger.Warn("messageReportReq after round completed", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgRound": msg.Round})
		return
	}
	err := repgen.verifyReportReq(msg)
	if err != nil {
		repgen.logger.Error("messageReportReq: could not validate report sent by leader", commontypes.logger.LogFields{
			"round": repgen.followerState.r,
			"error": err,
			"msg":   msg,
		})
	...
	if repgen.shouldReport(msg.AttributedSignedObservations) {
		...
		if err != nil {
			...
			repgen.logger.Error("messageReportReq: failed to sign report", commontypes.logger.LogFields{
				"round":  repgen.followerState.r,
				"error":  err,
				"id":     repgen.id,
				"report": report,
...

		{
			...
			if err != nil {
				repgen.logger.Error("could not verify my own signature", commontypes.logger.LogFields{
					"round":  repgen.followerState.r,
					"error":  err,
					"id":     repgen.id,
...
func (repgen *reportGenerationState) messageFinal(
	msg MessageFinal, sender commontypes.types.OracleID,
) {
	if msg.Epoch != repgen.e {
		repgen.logger.Debug("wrong epoch from MessageFinal", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgEpoch": msg.Epoch, "sender": sender})
		return
	}
	if msg.Round != repgen.followerState.r {
		repgen.logger.Debug("wrong round from MessageFinal", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgRound": msg.Round})
		return
	}
	if sender != repgen.l {
		repgen.logger.Warn("MessageFinal from non-leader", commontypes.logger.LogFields{
...
func (repgen *reportGenerationState) messageFinalEcho(msg MessageFinalEcho,
	sender commontypes.types.OracleID,
) {
	if msg.Epoch != repgen.e {
		repgen.logger.Debug("wrong epoch from MessageFinalEcho", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgEpoch": msg.Epoch, "sender": sender})
		return
	}
	if msg.Round != repgen.followerState.r {
		repgen.logger.Debug("wrong round from MessageFinalEcho", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "msgRound": msg.Round, "sender": sender})
		return
	}
	if repgen.followerState.receivedEcho[sender] {
		repgen.logger.Warn("extra MessageFinalEcho received", commontypes.logger.LogFields{
			"round": repgen.followerState.r, "sender": sender})
		return
	}
	if repgen.followerState.completedRound {
...
func (repgen *reportGenerationState) observeValue() observation.Observation {
	...
	if !ok {
		repgen.logger.Error("DataSource timed out", commontypes.logger.LogFields{
			"round":   repgen.followerState.r,
	...
	if err != nil {
		repgen.logger.ErrorIfNotCanceled("ReportGeneration: DataSource errored", repgen.ctx, commontypes.logger.LogFields{
			"round": repgen.followerState.r,
			"error": err,
		})
		return observation.Observation{}
...
func (repgen *reportGenerationState) shouldReport(observations []AttributedSignedObservation) bool {
	...
	if !ok {
		...
		repgen.logger.Error("shouldReport: blockchain interaction timed out, returning true", commontypes.logger.LogFields{
			"round":             repgen.followerState.r,
	...
	if resultTransmissionDetails.err != nil {
		repgen.logger.ErrorIfNotCanceled("shouldReport: Error during LatestTransmissionDetails", repgen.ctx, commontypes.logger.LogFields{
			"round": repgen.followerState.r,
	...
	if resultRoundRequested.err != nil {
		repgen.logger.Error("shouldReport: Error during LatestRoundRequested", commontypes.logger.LogFields{
			"round": repgen.followerState.r,
	...
	if err != nil {
		repgen.logger.Error("shouldReport: Error during observation.MakeObservation", commontypes.logger.LogFields{
			"round": repgen.followerState.r,
			"error": err,
		})
		return false
	...
	if override := repgen.configOverrider.ConfigOverride(); override != nil {
		repgen.logger.Debug("shouldReport: using overrides for alphaPPB and deltaC", commontypes.logger.LogFields{
			"round":             repgen.followerState.r,
			"alphaPPB":          alphaPPB,
			"deltaC":            deltaC,
	...
	logger := repgen.logger.MakeChild(commontypes.logger.LogFields{
		"round":                     repgen.followerState.r,
	...
	/*if initialRound {
		logger.Info("shouldReport: yes, because it's the first round of the first epoch", commontypes.logger.LogFields{
			"result": true,
		})
		return true
	}
	if deviation {
		logger.Info("shouldReport: yes, because new median deviates sufficiently from current onchain value", commontypes.logger.LogFields{
			"result": true,
		})
		return true
	}
	if deltaCTimeout {
		logger.Info("shouldReport: yes, because deltaC timeout since last onchain report", commontypes.logger.LogFields{
			"result": true,
		})
		return true
	}*/
	if unfulfilledRequest && zero_count>repgen.config.F {
		logger.Info("shouldReport: yes, because a new report has been explicitly requested", commontypes.logger.LogFields{
			"result": true,
		})
		return true
	}
	logger.Info("shouldReport: no", commontypes.logger.LogFields{"result": false})
	return false
}
...
func (repgen *reportGenerationState) completeRound() {
	repgen.logger.Debug("ReportGeneration: completed round", commontypes.logger.LogFields{
		"round": repgen.followerState.r,
	})
...
func (repgen *reportGenerationState) verifyReportReq(msg MessageReportReq) error {
	...
	{
		counted := map[commontypes.types.OracleID]bool{}
...
func (repgen *reportGenerationState) verifyAttestedReport(
	report AttestedReportMany, sender commontypes.types.OracleID,
) bool {
	if len(report.Signatures) <= repgen.config.F {
		repgen.logger.Warn("verifyAttestedReport: dropping final report because "+
			"it has too few signatures", commontypes.logger.LogFields{"sender": sender,
	...
	for oid, id := range repgen.config.OracleIdentities {
		keys[types.OnChainSigningAddress(id.OnChainSigningAddress)] =
			commontypes.types.OracleID(oid)
	}
